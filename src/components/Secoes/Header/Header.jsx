...
export default function Header({ timeLeft }) {
  const [backgroundImage, setBackgroundImage] = useState(imagemDesktop);

  useEffect(() => {
    function updateBackground() {
      setBackgroundImage(window.innerWidth < 500 ? imagemMobile : imagemDesktop);
    }
    updateBackground();
    ...
    <div>
      ...
      <div className="alerta-topo">
        ...
        <span className="contador">
          ...
          {timeLeft.days} dias,{' '}
          <span role="img" aria-label="Relógio de areia">
            ⌛
          </span>{' '}
          ...
          <div className="row header-row-fixa">
            ...
            <div>
              <h1 className="display-5 anton-regular corAmarela">
                ELIMINE <span className="color-white">ATÉ 10KG</span> EM <span>30 DIAS</span>{' '}
                COMO O MÉTODO QUE{' '}
                <span className="color-white">REALMENTE FUNCIONA.</span>
              </h1>
              <p className="lead mt-3 color-white texto-header">
                Você não precisa sofrer, passar fome ou viver na academia. O Método
                Evolution é <span className="corAmarela">leve, estratégico e se encaixa no seu dia a dia.</span>{' '}
                Mesmo que você não goste de academia. Mesmo que sua rotina seja corrida.
